...
def create_data_model(atm_information):
    ...
    data['vehicle_speed'] = 830 # = 50 km/h * 1000 km/m * 1/60 aprox 833 yup luego...
    data['depot'] = 0
...
def print_solution(data, manager, routing, assignment):  # pylint:disable=too-many-locals
    ...
    for node in range(routing.Size()):
        ...
        if assignment.Value(routing.NextVar(node)) == node:
            ...
            if (data['estimacion'][manager.IndexToNode(node)] == 0 and data['turno'][manager.IndexToNode(node)] == 0):
                stockout += 1
            if data['estimacion'][manager.IndexToNode(node)] == 0:
                dropped_1 += 1
            else:
                dropped_2 += 1

    # print(np.array(data['locations'])[dropped_nodes][0])
    solution["Objective"] = 0
    solution["Visitados"] = data["num_locations"] - dropped
    solution["Stockout"] = stockout
    solution["Dropped"] = dropped_nodes
    # print('cant stock out: ', stockout, dropped)
...
def main(dataframe):
    ...
    #print(dataframe)
    data = create_data_model(dataframe)
    ...
    # Allow to drop nodes by capacity
    for node in range(1, data['num_locations']):
        if data["estimacion"][node] == 0 and data["turno"][node] == 0:
            routing.AddDisjunction([manager.NodeToIndex(node)], 110000)
        elif data["estimacion"][node] == 0:
            routing.AddDisjunction([manager.NodeToIndex(node)], 105000)
        else:
